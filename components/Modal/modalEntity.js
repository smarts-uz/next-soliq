...
const Modal = (props) => {
    return (
        <div className={(props.show) ? 'fixed z-10 inset-0 overflow-y-auto' : 'fixed z-10 inset-0 hidden'}>
            <div className="flex items-end px-24 justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                    <div className="flex flex-col items-center">
                        <div className="flex justify-between  space-x-20 items-center bg-blue-50 p-12 font-sans text-2xl">
                            <p className="flex flex-col text-lg space-y-8">
                                <span>{props.data?.company_name}</span>
                                <span>{props.data?.adress}</span>
                            </p>
                            <div className="flex flex-col">
                                <p className="border-b-4 border-blue-900 bg-pink-500 text-sm w-64 rounded-t-xl p-4">gd Name: {props.data?.gd_name}</p>
                                <p className="bg-green-300  p-2 text-sm py-4">gb name: {props.data?.gb_name}</p>
                                <p className="bg-green-300 border-t-4 border-blue-900 rounded-b-xl p-2 text-sm py-4">gb tin: {props.data?.gb_tin}</p>
                            </div>
                        </div>
                    </div>
